...
Run this checklist first on the local RHEL 9 VM, then on the AWS RHEL 9
test server. Use the same reviewed revision on both. This round covers Codex,
OpenCode, and Claude Code; Switchyard is the next phase.
...
Complete the no-key [local RHEL VM checks](rhel-vm.md) first. On AWS, start
with the [in-memory quickstart](../quickstarts/in-memory.md). The Fedora
...
Provide two separate ordinary user accounts and their SSH access. Neither
...
Follow the [user workflow](../user-workflow.md) to log in, install the harness
under that user's home, and configure its loopback endpoint with the
non-secret placeholder. Do not log into the harness with a personal provider
...
Follow the [Valkey quickstart](../quickstarts/valkey.md), reusing the real `v1`
...
purge Valkey data during persistence testing.
